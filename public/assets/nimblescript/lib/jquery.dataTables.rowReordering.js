...
(function ($)
{
    ...
    $.fn.rowReordering = function (options)
    {
        ...
        function fnMoveRows(sSelector, iCurrentPosition, iNewPosition, sDirection, row, sGroup)
        {
                    ...
                    } else
                    {
                        if (sDirection == "back")
                        {
                            ...
                            oTable.fnUpdate(iRowPosition + 1,
								        ...
								        false,
                                        false); // false = defer redraw until all row updates are done
                            // tr._DT_RowIndex = iRowPosition + 1;
                        } else
                        {
                            oTable.fnUpdate(iRowPosition - 1,
								        oTable.fnGetPosition(tr), // get row position in current model
            ...
            // Below code seems to be un-necessary
        ...
        return this.each(function ()
        {
            ...
            $("tbody", oTable).sortable({
                ...
                forceHelperSize: properties.helper ? true : false,
                cursor: "move",
                update: function (event, ui)
                {
                    ...
                    var oState = fnGetState(sSelector, ui.item);
