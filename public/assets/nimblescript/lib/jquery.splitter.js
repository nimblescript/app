    ...
    return this.each(function ()
    {
        ...
        var bar = $('<div></div>')
            ...
            .css({position: "absolute", "user-select": "none", "-webkit-user-select": "none",
                "-khtml-user-select": "none", "-moz-user-select": "none", "z-index": "2"})
		...
		if ( opts.cookie ) {
			...
			var cookieInitPos = parseInt($.cookie(opts.cookie),10);
		...
		if ( opts.resizeTo ) {
			...
			{
			    var top = 0; // splitter.offset().top;
			    var eh = $(opts.resizeTo).height();
			    var height = (opts.height || Math.max(eh - top - splitter._hadjust, splitter._hmin) ) + "px";
			    debug('Height', height, 'eh',eh, 'top',top, 'hadjust', splitter._hadjust, 'hmin',splitter._hmin);
